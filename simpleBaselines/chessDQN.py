...
# Create the agent
DQN_Agent = DeterministicDQN_RL_Agent(env=env, seed=seed)

# Play the game
for episode in range(num_episodes):

	DQN_Agent.reset_env(seed=seed)

	# Play the game (new environment for each run with continuously learning agent)
	DQN_Agent.play(max_steps=5000, seed=seed)

	rewards_total.append(DQN_Agent.final_state.cumulative_reward)
	steps_total.append(DQN_Agent.final_state.step + 1)
	egreedy_total.append(DQN_Agent.egreedy)
